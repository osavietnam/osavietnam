...
export async function renderProvinceNewsPage(page: number, requestedTheme = 'dark', openExternal = false) {
  ...
  const firstPageHref = `${deploymentBase}/api/province-news.html?theme=${theme}${modeParam}`;
  const pageHref = (number: number) => number === 1 ? firstPageHref : `${deploymentBase}/api/province-news-page/${number}.html?theme=${theme}${modeParam}`;
  const articleHref = (link: string) => openExternal || import.meta.env.GITHUB_PAGES === 'true'
    ? link
  ...
  const cards = result.items.map((item) => `<a class="province-news-card" href="${escapeHtml(articleHref(item.link))}"${externalAttrs}>
  ...
  const css = `
html,body{margin:0!important;width:100%;background:#08111f;color:#f1e8d8;font-family:'Be Vietnam Pro',Arial,sans-serif;color-scheme:dark}*{box-sizing:border-box}html{scrollbar-width:thin;scrollbar-color:#c5a059 transparent}::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{border:0;border-radius:0;background:#c5a059}::-webkit-scrollbar-thumb:hover{background:#e9c176}body{min-height:0}
...
.province-news-card{display:flex;width:100%;min-width:0;flex-direction:column;overflow:hidden;background:#101b30;border:1px solid rgba(233,193,118,.18);color:inherit;text-decoration:none;transition:transform .3s cubic-bezier(.22,1,.36,1),background-color .25s ease,border-color .25s ease}
.province-news-card img,.province-news-card-placeholder{display:block;width:100%;aspect-ratio:16/9;height:auto;object-fit:cover;background:#16233b;opacity:.94;transition:opacity .25s ease,transform .4s cubic-bezier(.2,.75,.25,1)}
