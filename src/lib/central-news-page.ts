...
export async function renderCentralNewsPage(page: number, origin: string, requestedTheme = 'dark') {
  ...
  const local = `${deploymentBase}/api/central-news.html?theme=${theme}`;
  ...
  const cards = items.map((item) => {
    ...
    return `<a class="central-news-card" href="${escapeHtml(articleHref)}">
  ...
  const pageHref = (pageNumber: number) => pageNumber === 1 ? local : `${paged}/${pageNumber}.html?theme=${theme}`;
...
.central-news-card{display:flex;align-self:center;width:100%;max-width:none;flex-direction:column;min-width:0;overflow:hidden;background:#101c31;border:1px solid rgba(233,193,118,.1);color:inherit;text-decoration:none;transition:transform .65s cubic-bezier(.22,1,.36,1),background-color .65s ease,border-color .4s ease}
.central-news-card img,.central-news-card-placeholder{display:block;width:100%;aspect-ratio:16/9;height:auto;object-fit:cover;background:#16233b;opacity:.9;transition:opacity .65s ease,transform .8s cubic-bezier(.2,.75,.25,1)}
.central-news-card-copy{padding:16px 17px 18px;text-align:left;transition:background-color .65s ease}
.central-news-card time{display:block;color:#c5a059;font-size:11px;letter-spacing:.08em;text-transform:uppercase;opacity:.95}
.central-news-card h3{margin:8px 0;color:#efe7d6;font:600 17px/1.35 'Playfair Display',Georgia,serif;transition:color .55s ease,transform .55s cubic-bezier(.22,1,.36,1)}
